...
/*WIFI configuration*/
#define ESP_WIFI_SSID      "aPanMateusz"
#define ESP_WIFI_PASS      "hAslo1313@@"
#define ESP_MAXIMUM_RETRY  10

/*Telegram configuration*/
#define TOKEN "5015722781:AAFHUAVGupQZ1Dtcy5HdeN3XrWR_SDLtDzs"
...
#define chat_ID1 "374148396"
#define chat_ID2 "374148396"
...
message_struct mess_str;

static void cJSON_parser_GetMe(char *output_buffer);
...
static void https_telegram_cleanUpdates()
{
    	char url[512] = "";
    ...
    ESP_LOGW(TAG2, "updata_id_send: %d",mess_str.updata_id);
	sprintf(post_data,"{\"offset\":%d}", mess_str.updata_id );
    // sprintf(post_data,"{\"offset\":-1}" );
    ...
    if (err == ESP_OK) {
    ESP_LOGW(TAG2, "Updates: %s",output_buffer);

    cJSON_parser_GetMe(&output_buffer);
    }
    else {
...
static void https_telegram_sendMessage_echo_perform_post(char *echoText) {
	...
	char post_data[512] = "";
        sprintf(post_data,"{\"chat_id\":%s,\"text\":\"%s - OK\"}",chat_ID2,echoText);
        ESP_LOGW(TAG2, "post_data: %s",post_data);
    ////ESP_LOGW(TAG, "El json es es: %s",post_data);
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_http_client_set_post_field(client, post_data, strlen(post_data));
    ...
    if (err == ESP_OK) {
        ...
        ESP_LOGW(TAG3, "sendMessage output: %s",output_buffer);
...
static void cJSON_parser_GetMe(char *output_buffer)
{
    ...
    if (root == NULL) 
    {
        ESP_LOGW(TAG2, "errorPars");
        return 0;
    }
    
    if(cJSON_GetObjectItem(root, "ok")->valueint )
    {
        cJSON *result  = cJSON_GetObjectItem(root, "result");
        int size_tab = cJSON_GetArraySize(result);
        ESP_LOGW(TAG2, "size: %d", size_tab);
        if (size_tab)
        {
            cJSON *onearray = cJSON_GetArrayItem(result,size_tab-1);
            mess_str.updata_id  = cJSON_GetObjectItem(onearray, "update_id")->valueint;
            ESP_LOGW(TAG2, "id: %d", mess_str.updata_id );
            
            cJSON *message = cJSON_GetObjectItem(onearray, "message");
            cJSON *from = cJSON_GetObjectItem(message, "from");
            mess_str.id  = cJSON_GetObjectItem(from, "id")->valueint;
            ESP_LOGW(TAG2, "mes_id: %d", mess_str.id);
            
            cJSON *chat = cJSON_GetObjectItem(message, "text");
            memcpy(mess_str.data_text, chat->valuestring, strlen(chat->valuestring));

            https_telegram_sendMessage_echo_perform_post(chat->valuestring);

            ESP_LOGW(TAG2, "text: %s", mess_str.data_text);

            mess_str.updata_id++;
        }    
         else
        {
         ESP_LOGW(TAG2, "result array=0");
        } 
    }   
    cJSON_Delete(root);
    ESP_LOGI(TAG2, "___________");
...
static void https_telegram_sendMessage_perform_post(void) {
	...
	*/
	...
	//const char *post_data = "{\"chat_id\":852596694,\"text\":\"Envio de post\"}";
	char post_data[512] = "";
	sprintf(post_data,"{\"chat_id\":%s,\"text\":\"read\",\"reply_markup\":{\"keyboard\":[[{\"text\":\"button1\"}],[{\"text\":\"button2\"}]]}}",chat_ID2);
    ////ESP_LOGW(TAG, "El json es es: %s",post_data);
    esp_http_client_set_method(client, HTTP_METHOD_POST);
    esp_http_client_set_header(client, "Content-Type", "application/json");
    esp_http_client_set_post_field(client, post_data, strlen(post_data));
